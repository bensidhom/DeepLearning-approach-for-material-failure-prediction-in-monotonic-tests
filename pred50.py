...
model = load_model(r'C:\all\data/Force_window.keras')
...
for i in range (1, len(X_test)-1):
    ...
    reverse_curr = pd.DataFrame({'force': [reverse_minmax(x) for x in X_test[i]]})

    points_to_predict = 50
    ...
    reverse_future = pd.DataFrame({'force': [reverse_minmax(x) for x in future]})

    plt.scatter(550,reverse_future['force'][0], label=' First prediction',color='black') 
    plt.legend()
    plt.plot(numbers,reverse_curr_frame, label='True',color='green')    
    plt.legend()
    plt.plot(numbers,reverse_future,label='prediction',color='red')  
    plt.legend()
